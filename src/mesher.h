...
public:
  ...
  void get_tex_coords(Block_Type type, int face, float &u_min, float &v_min,
                      float &u_max, float &v_max);
  void add_face(std::vector<float> &vertices, float x, float y, float z,
                int face_index, Block_Type block);
  void generate_mesh();
  void generate_full_mesh();
  bool should_mesh_face(
      const glm::ivec3 &chunk_pos, const glm::ivec3 &face_pos,
      const glm::ivec3 &direction); // takes neighboring chunks into account
                                    // when face is at chunk edge
  void upload_mesh();
  void clear_mesh();
  bool is_dirty();
  void mark_clean();
  ...
  unsigned int get_skybox_VAO();
  unsigned int get_skybox_VBO();
...
Mesher::Mesher(World &world)
    : world_(world), VAO_(0), VBO_(0), vertex_count_(0), is_dirty_(false) {}

Mesher::~Mesher() { clear_mesh(); }
...
void Mesher::get_tex_coords(Block_Type type, int face, float &u_min,
...
void Mesher::add_face(std::vector<float> &vertices, float x, float y, float z,
                      int face_index, Block_Type block) {
...
void Mesher::generate_mesh() {
  ...
  for (auto &[chunk_pos, chunk] : world_.chunks_) {
    ...
    glm::ivec3 chunk_origin(chunk_pos.x * CHUNK_WIDTH,
                            chunk_pos.y * CHUNK_HEIGHT,
                            chunk_pos.z * CHUNK_WIDTH);

    float world_x = static_cast<float>(chunk_pos.x * CHUNK_WIDTH);
    float world_y = static_cast<float>(chunk_pos.y * CHUNK_HEIGHT);
    float world_z = static_cast<float>(chunk_pos.z * CHUNK_WIDTH);

    for (int x = 0; x < CHUNK_WIDTH; x++) {
      for (int y = 0; y < CHUNK_HEIGHT; y++) {
        for (int z = 0; z < CHUNK_WIDTH; z++) {
          Block_Type block = static_cast<Block_Type>(
              chunk.blocks_[index(glm::ivec3(x, y, z))]);
          if (block == AIR) {
          ...
          glm::ivec3 world_pos(chunk_origin.x + local_pos.x,
                               chunk_origin.y + local_pos.y,
                               chunk_origin.z + local_pos.z);
          float wx = world_x + static_cast<float>(x);
          float wy = world_y + static_cast<float>(y);
          float wz = world_z + static_cast<float>(z);

          // Check each face and add vertices if visible... condition for
          // visibility is whether surface is exposed to air Face 0: Back (-Z)
...
void Mesher::generate_full_mesh() {
  ...
  for (auto &[position, chunk] : world_.chunks_) {
    world_x = position.x * CHUNK_WIDTH;
    world_y = position.y * CHUNK_WIDTH;
    world_z = position.z * CHUNK_WIDTH;
    for (int x = 0; x < CHUNK_WIDTH; x++) {
...
bool Mesher::should_mesh_face(const glm::ivec3 &chunk_pos,
                              const glm::ivec3 &local_pos,
                              const glm::ivec3 &direction) {
  glm::ivec3 neighbor_local = local_pos + direction;
  if (neighbor_local.x >= 0 && neighbor_local.x < CHUNK_WIDTH &&
      neighbor_local.y >= 0 && neighbor_local.y < CHUNK_HEIGHT &&
      neighbor_local.z >= 0 && neighbor_local.z < CHUNK_WIDTH) {
    // case neighboring block is in same chunk
    uint16_t neighbor_block =
        world_.chunks_[chunk_pos].blocks_[index(neighbor_local)];
    return neighbor_block == AIR;
  }
  // else neighboring block is in a different chunk
  glm::ivec3 neighbor_chunk_pos =
      chunk_pos + direction; // direction is a unit vector so is context
                             // agnostic in discrete spaces
  auto it = world_.chunks_.find(neighbor_chunk_pos);
  if (it == world_.chunks_.end()) {
    return true; // neighboring chunk doesn't exist
  }
  glm::ivec3 converted_local(
      (neighbor_local.x + CHUNK_WIDTH) % CHUNK_WIDTH,
      (neighbor_local.y + CHUNK_HEIGHT) % CHUNK_HEIGHT,
      (neighbor_local.z + CHUNK_WIDTH) %
          CHUNK_WIDTH); // neighbor_local is relative to current chunk;
                        // conversion to neighbor chunk's block space
  uint16_t neighbor_block = it->second.blocks_[index(converted_local)];
  return neighbor_block == AIR;
}

void Mesher::upload_mesh() {
  vertex_count_ = mesh_vertices_.size() / 8; // 5 floats per vertex (3pos +
                                             // 2tex)

  glBindVertexArray(VAO_);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_);
  glBufferData(GL_ARRAY_BUFFER, mesh_vertices_.size() * sizeof(float),
...
std::size_t Mesher::get_vertex_count() { return vertex_count_; }
MeshStats Mesher::get_mesh_stats() { return stats_; }
unsigned int Mesher::get_VAO() { return VAO_; }
unsigned int Mesher::get_VBO() { return VBO_; }
unsigned int Mesher::get_skybox_VAO() { return skybox_VAO_; }
unsigned int Mesher::get_skybox_VBO() { return skybox_VBO_; }
bool Mesher::is_dirty() { return is_dirty_; }
void Mesher::mark_clean() { is_dirty_ = false; }